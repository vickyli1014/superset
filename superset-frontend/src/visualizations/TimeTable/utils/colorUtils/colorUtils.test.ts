...
test('colorFromBounds should return null when no bounds are provided', () => {
  expect(colorFromBounds(50)).toBeNull();
  expect(colorFromBounds(50, undefined)).toBeNull();
});

test('colorFromBounds should return null when bounds is empty array', () => {
  expect(colorFromBounds(50, [])).toBeNull();
});

test('colorFromBounds should handle min and max bounds with color scale', () => {
...
test('colorFromBounds should handle only min bound', () => {
...
test('colorFromBounds should handle only max bound', () => {
...
test('colorFromBounds should handle null value with min bound', () => {
  const result = colorFromBounds(null, [30, null]);
  expect(result).toBe('#ca0020'); // minColor when value is null
});

test('colorFromBounds should handle null value with max bound', () => {
  const result = colorFromBounds(null, [null, 70]);
  expect(result).toBe('#ca0020'); // minColor when value is null
});

test('colorFromBounds should handle custom color bounds', () => {
...
test('colorFromBounds should handle edge case with min equals max', () => {
...
test('colorFromBounds should handle value of 0', () => {
...
test('colorFromBounds should handle negative values', () => {
